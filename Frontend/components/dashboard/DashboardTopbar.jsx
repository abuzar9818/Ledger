...
function DashboardTopbar({ title = "Dashboard", subtitle = "Track your accounts and balances in real time." }) {
  ...
  return (
    <header className="relative z-[70] border-b border-slate-200/90 bg-white/80 px-4 py-4 backdrop-blur-sm sm:px-6">
      <div className="flex items-center justify-between gap-3 overflow-visible">
        ...
        <div className="flex items-center gap-4 relative z-[80]">
          ...
          >
            ...
            <span className="truncate font-semibold">{user?.name || "Profile"}</span>
          </button>

          {open ? (
            <div className="absolute right-0 top-[calc(100%+12px)] z-[90] w-72 rounded-xl border border-slate-200 bg-white p-3 shadow-[0_18px_45px_rgba(15,23,42,0.14)] backdrop-blur-sm">
