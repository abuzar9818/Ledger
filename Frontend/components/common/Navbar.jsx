    ...
    <header className="sticky top-0 z-40 border-b border-slate-200/70 bg-white/80 text-slate-900 backdrop-blur-md">
      <nav className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 sm:px-6">
        ...
        <div className="hidden items-center gap-1 text-sm font-medium md:flex">
          <Link to="/" className="rounded-md px-3 py-2 text-slate-700 transition hover:bg-slate-100 hover:text-slate-900">
            Home
          </Link>
          ...
          {isAuthenticated ? (
            <>
              ...
              <button
                ...
                className="ml-1 rounded-md border border-slate-300 bg-white px-3 py-2 text-slate-700 transition hover:border-rose-300 hover:bg-rose-50 hover:text-rose-700"
              >
                Logout
              </button>
            </>
            ...
            <Link
              to="/auth/register"
              className="ml-1 rounded-md bg-gradient-to-r from-teal-500 to-amber-400 px-3 py-2 font-semibold text-slate-900 transition hover:from-teal-400 hover:to-amber-300"
            >
              Start Free
            </Link>
          )}
        ...
        <div className="border-t border-slate-200 bg-white px-4 py-3 md:hidden">
          <div className="flex flex-col gap-2 text-sm font-medium">
            ...
            {isAuthenticated ? (
              <>
                {userLinks.map((link) => (
                  <Link key={link.to} to={link.to} onClick={closeMobileMenu} className="rounded-md px-3 py-2 text-slate-700 transition hover:bg-slate-100">
                    {link.label}
                  </Link>
                ))}
                {user?.role === "ADMIN" ? (
